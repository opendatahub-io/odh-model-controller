...
func (s *ConfigMapReferenceSource) isReferencedLocked(ref types.NamespacedName) bool {
	for _, currentRef := range s.references {
		if currentRef == ref {
			return true
		}
	}
...
func (s *ConfigMapReferenceSource) startReference(ref types.NamespacedName) error {
	...
	informerCtx, cancel := context.WithCancel(ctx)
	s.mu.Lock()
	if _, exists := s.informers[ref]; exists || !s.isReferencedLocked(ref) {
		cancel()
		s.mu.Unlock()
		return nil
	}
...
func (s *ConfigMapReferenceSource) stopUnreferenced() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	for ref, informer := range s.informers {
		if s.isReferencedLocked(ref) {
			continue
		}
		informer.cancel()
