...
const (
	...
	rayTlsScriptsPath           = "./testdata/configmaps/ray-tls-script.yaml"
	rayTlsScriptsUpdatedPath    = "./testdata/configmaps/ray-tls-script-updated.yaml"
...
var _ = Describe("KServe Ray TLS controller", func() {
	...
	Context("when a non-multinode ServingRuntime created", func() {
		It("should not create a 'ray-ca-cert' Secret and 'ray-tls-script' ConfigMap in the testNs", func() {
			testNamespace := Namespaces.Create(cli)
			testNs := testNamespace.Name

			// Create ray tls script in the ctrl namespace
			...
			// Check if all ray tls script are NOT created in the testNs
			...
			secret, err := waitForSecret(cli, testNs, constants.RayCASecretName, 3, 1*time.Second)
			Expect(err).To(HaveOccurred())
	...
	Context("when a multinode ServingRuntime created", func() {
		It("should create a 'ray-ca-cert' Secret and 'ray-tls-script' ConfigMap in the testNs where the SR exist", func() {
			testNamespace := Namespaces.Create(cli)
			testNs := testNamespace.Name
			...
			// Check if all ray tls script are created in the testNs
	...
	Context("when a multinode ServingRuntime exists", func() {
		...
		It("should recreate a 'ray-ca-cert' Secret when it is removed manually", func() {
			...
			err := cli.Get(ctx, types.NamespacedName{Name: constants.RayCASecretName, Namespace: testNs}, secret)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should rollback 'ray-ca-cert' Secret in the target ns when it is changed", func() {
			...
			originalRayCaCertSecret, err := waitForSecret(cli, WorkingNamespace, constants.RayCASecretName, 30, 1*time.Second)
			Expect(err).NotTo(HaveOccurred())
			Eventually(func() bool {
				updatedSecretFromTestNs, err := waitForSecret(cli, testNs, constants.RayCASecretName, 1, 1*time.Second)
				Expect(err).NotTo(HaveOccurred())
				return compareSecrets(originalRayCaCertSecret, updatedSecretFromTestNs)
			}, timeout, interval).Should(BeTrue())
		})
		It("should create a 'ray-tls-script' ConfigMap when it is removed manually", func() {
			configMap := &corev1.ConfigMap{}
			err := cli.Get(ctx, types.NamespacedName{Name: constants.RayTlsScriptConfigMapName, Namespace: testNs}, configMap)
			Expect(err).NotTo(HaveOccurred())

			By("deleting a 'ray-tls-script' configMap in the testNs")
			Expect(cli.Delete(ctx, configMap)).To(Succeed())

			// Check if 'ray-tls-script' ConfigMap is recreated
			_, err = waitForConfigMap(cli, testNs, constants.RayTlsScriptConfigMapName, 30, 1*time.Second)
			Expect(err).NotTo(HaveOccurred())
		})
		It("should rollback 'ray-tls-script' ConfigMap in the target ns when it is changed", func() {
			By("updating existing 'ray-tls-script' ConfigMap in the testNs")
		...
		It("should 'ray-tls-script' ConfigMap in the testNs when original one in the ctrlNs updated", func() {
			By("updating original 'ray-tls-script' ConfigMap in the ctrlNs")
			rayTlsScriptsUpdatedConfigMap := &corev1.ConfigMap{}
			err := convertToStructuredResource(rayTlsScriptsUpdatedPath, rayTlsScriptsUpdatedConfigMap)
			Expect(err).NotTo(HaveOccurred())
			...
			// Check if 'ray-tls-script' ConfigMap in the testNs is updated.
			Eventually(func() bool {
				updatedConfigMapFromTestNs, err := waitForConfigMap(cli, testNs, constants.RayTlsScriptConfigMapName, 1, 1*time.Second)
				Expect(err).NotTo(HaveOccurred())
		...
		It("should update a 'ray-ca-cert' Secret in the testNs when original one in the ctrlNs updated", func() {
			...
			_, err = waitForSecret(cli, WorkingNamespace, constants.RayCASecretName, 30, 1*time.Second)
			Expect(err).NotTo(HaveOccurred())
	...
	Context("when a multinode ServingRuntime removed from the testNs", func() {
		...
		It("ray tls script should not be removed if there is a multinode ServingRuntime in the testNs", func() {
		...
		It("ray tls script should be removed if there is no multinode ServingRuntime in the testNs", func() {
			...
			// Check if all ray tls script are removed
			configmap, err := waitForConfigMap(cli, testNs, constants.RayTlsScriptConfigMapName, 3, 1*time.Second)
			Expect(err).To(HaveOccurred())
			Expect(configmap).To(BeNil())
