...
type ServiceMeshMemberRollHandler interface {
	...
	RemoveMemberFromSMMR(ctx context.Context, key types.NamespacedName, member string) error
}
...
func (r *serviceMeshMemberRollHandler) RemoveMemberFromSMMR(ctx context.Context, key types.NamespacedName, member string) error {
	smmr := &v1.ServiceMeshMemberRoll{}
	err := r.client.Get(ctx, key, smmr)
	if err != nil {
	...
	serviceMeshMemberList := smmr.Spec.Members
	// remove the namespace from the list
	for i, smmrMember := range serviceMeshMemberList {
		if smmrMember == member {
			serviceMeshMemberList = append(serviceMeshMemberList[:i], serviceMeshMemberList[i+1:]...)
		}
	}
	smmr.Spec.Members = serviceMeshMemberList
	if err = r.client.Update(ctx, smmr); err != nil {
		return fmt.Errorf("failed to remove member from ServiceMeshMemberRole: %w", err)
	}
	return nil
}
