...
const (
	...
	targetFolder  = "./controllers/testdata/nim/benchmark_documents"
)
...
func main() {
	...
	for r := 1; r <= *runtimes; r++ {
		...
		if len(resources) == *pageSize || r == *runtimes {
			...
			if err := os.WriteFile(fmt.Sprintf(targetFileFmt, targetFolder, *runtimes, *pageSize, currentPage), b, os.ModePerm); err != nil {
				panic(err)
			}
			resources = []Resource{}
			currentPage++
...
func makeDummyResource(name, resourceId string) Resource {
	return Resource{
		...
		Description:     "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Proin viverra felis arcu, a fringilla mi posuere non. Quisque tristique justo non nibh tincidunt euismod. Fusce placerat consectetur tempus",
