...
server serves them as static, extensively commented YAML files that can help the user make the right choice. Those files
are currently embedded at build time.
...
- Extracts optional `topology` query parameter — when `type=router` and topology ends with `-pd`, resolves to the P/D
  router sample
